<template>
    <nav class="flex justify-between w-full px-16 py-5 top-0 fixed bg-[#030a09] z-50" data-aos="flip-up" data-aos-easing="ease-in-quart" data-aos-duration="700">
        ...
        <div class="absolute left-0 w-0 opacity-0 bg-[#030a09] h-screen z-50 py-7 px-8 duration-300 ease-linear" :class="showSidebar ? '!w-64 !opacity-100' : ''">
            ...
            <div class="flex flex-col gap-12 opacity-0 duration-300 ease-in-out" :class="showSidebar ? '!opacity-100' : ''">
                <a :class="showSidebar ? '!delay-100 !translate-x-0 opacity-100' : ''" @click="sideScroll('intro')" class="translate-x-24 delay-0 text-[15px] font-epilogue w-fit font-light after:w-6 after:transition-all after:duration-300 after:ease-linear after:opacity- hover:after:w-full hover:after:opacity-100 after:bg-neon-green after:bg-opacity-70 after:h-[0.4px] after:block nav-item cursor-pointer duration-500 opacity-0 intro active" data-aos-easing="ease-in-cubic">Introduction</a>
